  ...
  class AccountActiveDirectoryFolderCreateWorker
    ...
    def perform(account_id)
      ...
      if ENV["TRYGRID_AD_ENABLED"].to_b
        ...
        begin
          create_dir_url = "#{ENV['TRYGRID_API_HOST']}/createdir"
          response = RestClient.post(url=create_dir_url,payload={:username => @account.username, :region => region}, headers= {:token => ENV["TRYGRID_API_KEY"]})
          puts response.body
        rescue => e
          puts e
        end
        ...
        rem_regions.each do |sync_reg|
          begin
            create_dir_url = "#{ENV['TRYGRID_API_HOST']}/createdir"
            response = RestClient.post(url=create_dir_url,payload={:username => @account.username, :region => sync_reg}, headers= {:token => ENV["TRYGRID_API_KEY"]})
            puts response.body
          rescue => e
            puts e
          end
        ...
        Ghetto::AccountActiveDirectoryHorizonSyncWorker.perform_async
    end
  end
end